...
Model.prototype.validate = function()
{
    ...
    {
        self._validate(self.$$schema, self.$$values);
    });
}; // end validate

// Inner validation function
Model.prototype._validate = function(schema, values)
{
    var self = this;
    function fail(key, expectedType, message)
    {
        throw new errors.ValidationError(key, expectedType, message);
    } // end fail
    ...
    _.forIn(schema, function(type, key)
    {
        ...
        var val = _.has(self.$$updates, key) ? self.$$updates[key] : values[key];

        // Support option objects instead of just plain types, as well as nesting
        ...
        else if(_.isPlainObject(type))
        {
            ...
            return self._validate(type, val);
        }
