...
class TDB extends EventEmitter
{
    ...
    _getNextExp()
    {
        const nextTimestamp = _(this._expirations).keys().sortBy().first();
    ...
    _checkExpiration()
    {
        ...
        _(this._expirations).keys().sortBy()
            ...
            .each((timestamp) =>
            {
                const keys = this._expirations[timestamp];
        ...
        const nextExp = this._getNextExp();
        if(nextExp)
        {
    ...
    {
        const key = this.set(name, value, expiration);
        return this.sync()
            .then(() => key);
    } // end save
